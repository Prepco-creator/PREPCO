...
const Navbar = () => {
    ...
    return (
        <nav className={`${isMenuOpen ? 'bg-primary text-white' : 'glass-morph-effect'} rounded-br-3xl rounded-bl-3xl fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600 `} >
            <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                ...
                <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
                    <button
                        type="button"
                        className="p-2 rounded-lg text-white bg-gradient-to-r from-primary to-secondary"
                    >
                        Contact us
                    </button>
                    <button
                        ...
                        {navLinks.map(({ title, route }) => (
                            <li key={route}>
                                <Link href={route} passHref>
                                    <div
                                        className={`capitalize block py-2 px-3 rounded md:bg-transparent md:p-0 hover:text-primary ${pathname === route ? "text-primary" : "text-gray-900"
                                            }`}
